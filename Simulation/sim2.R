...
get_data = function(seed = 123){
  set.seed(seed)
  allA = matrix(NA, nrow = 1200, ncol = 3)
  allX = c()
...
while(nsim<=Nsim){
  ...
  noise1 = rnorm(1000, mean = 0, sd = 0.05)
...
train2 = get_data(123)
test2 = get_data(321)
write.csv(train2, 'SimData/train2.csv')
write.csv(test2, 'SimData/test2.csv')

set.seed(321)
raw = read.csv('SimData/test2.csv', row.names = 1)


raw_s = read.csv('SimData/sim2raw.csv', header = F)
s = t(read.csv('SimData/sim2va.csv', header = F))
...
sim2 = data.frame(VA = e1va, fastICA = e1raw)
boxplot(sim2)
