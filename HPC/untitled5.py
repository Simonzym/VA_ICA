  ...
  def __init__(self, latent_dim):
    ...
    self.decoder = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.Dense(256, activation='relu', kernel_regularizer = l2(0.01)),
            # No activation
            tf.keras.layers.Dense(ic_size*2, activation='tanh', kernel_regularizer = l2(0.01)),
        ]
    )
...
np.savetxt("simHPC/mean4.csv", means, delimiter=",")
np.savetxt('simHPC/test4.csv', test, delimiter = ',')
#use fastICA to extract mixing matrix
from sklearn.decomposition import FastICA     
va_transformer = FastICA(n_components = 5, random_state = 0)
va_features = va_transformer.fit_transform(means.numpy().T)
...
np.savetxt("simHPC/sim4va.csv", va_features, delimiter=",")

raw_mixing = np.array(raw_transformer.components_)
np.savetxt("simHPC/sim4raw.csv", raw_features, delimiter=",")


np.savetxt("simHPC/sim4vamix.csv", va_mixing, delimiter=",")
np.savetxt("simHPC/sim4rawmix.csv", raw_mixing, delimiter=",")
