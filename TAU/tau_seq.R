...
tau_path = 'Y:/PhD/Fall 2020/RA/Data/MRI_TAU9_3'
...
setwd('Y:/')
for(i in names(image_names)){
  num = length(image_names[[i]])
  new_folder = paste0('TAU/', i, '/')
  cpath = paste0(tau_path, '/', i)
...
tau_result = readRDS('Y:/PhD/Thesis3/IC25/multi_tau.rds')
weights = tau_result$A
...
dt25 = data.frame(ic4 = weights[,4]*1000, ic11 = weights[,11]*1000, ic13 = weights[,12]*1000,
...
dd.p13 = ggplot(multi.25) + 
  geom_line(aes(lag_time, ic13, group = PTID), alpha = 0.3) +
  geom_line(data = dd, aes(x = lag_time, y = ic13, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 13",
...
dd.p11 = ggplot(multi.25) + 
  geom_line(aes(lag_time, ic11, group = PTID), alpha = 0.2) +
  geom_line(data = dd, aes(x = lag_time, y = ic11, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 11",
    x = 'Time from First Scan (months)',
...
dd.p4 = ggplot(multi.25) + 
  geom_line(aes(lag_time, ic4, group = PTID), alpha = 0.3) +
  geom_line(data = dd, aes(x = lag_time, y = ic4, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 4",
    x = 'Time from First Scan (months)',
...
dd.p19 = ggplot(multi.25) + 
  geom_line(aes(lag_time, ic19, group = PTID), alpha = 0.3) +
  geom_line(data = dd, aes(x = lag_time, y = ic19, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 19",
    x = 'Time from First Scan (months)',
  ...
  theme(plot.title = element_text(hjust = 0.5), axis.line = element_line(colour = "black"),
        ...
        panel.border = element_blank(),
        panel.background = element_blank())
ggarrange(dd.p4, dd.p11, dd.p13, dd.p19, common.legend = T)
...
p13 = ggplot(multi.info) + 
  geom_line(aes(lag_time, ic13, group = PTID), alpha = 0.3) +
  geom_line(data = md, aes(x = lag_time, y = ic13, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 13",
    x = 'Time from First Scan (months)',
...
p16 = ggplot(multi.info) + 
  geom_line(aes(lag_time, ic16, group = PTID), alpha = 0.2) +
  geom_line(data = md, aes(x = lag_time, y = ic16, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 16",
    x = 'Time from First Scan (months)',
...
p23 = ggplot(multi.info) + 
  geom_line(aes(lag_time, ic23, group = PTID), alpha = 0.3) +
  geom_line(data = md, aes(x = lag_time, y = ic23, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 23",
    x = 'Time from First Scan (months)',
...
p20 = ggplot(multi.info) + 
  geom_line(aes(lag_time, ic20, group = PTID), alpha = 0.3) +
  geom_line(data = md, aes(x = lag_time, y = ic20, color = as.factor(DXCURREN)), alpha = .8, size = 1)+
  theme_bw() +
  labs(
    title = "IC 20",
    x = 'Time from First Scan (months)',
